<x-app-layout>
    ...
    <form action="{{ route('peserta.store') }}" method="POST" class="max-w-sm mx-auto">
        @csrf
        @method('POST')
        <div class="mb-5">
          <label for="nm_peserta" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"> Nama Peserta</label>
          <input type="text" id="nm_peserta" name="nm_peserta" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="" required>
        </div>
        <div class="mb-5">
        ...
        @foreach ($skema as $item)
            <option value="{{$item->kd_skema}}">{{$item->kd_skema}}</option>
        @endforeach  
        </select>
        </div>
        <div class="mb-5">
        ...
        <div class="mb-5">
          ...
          <input type="number" id="nik" name="nik" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="" required>
        </div>
